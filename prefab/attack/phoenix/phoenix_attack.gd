...
func _process(delta):
	...
	for body in corpos:
		...
		if foi_refletida and body.has_method("take_damage") and body != owner:
			...
			if "Boss" in body.name or body.is_in_group("boss") or body.has_node("FireballSpawn"):
		...
		if not foi_refletida and body is CharacterBody2D and body.has_method("handle_fireball_collision"):
...
func _processa_colisao(body):
	if body.has_method("handle_fireball_collision"):
		if body.is_shielding:
			foi_refletida = true
			body.handle_fireball_collision(self)
